...
parser.add_argument('--batch_size', '-b', type=int, default=192, help='Batch size for training.')
...
if add_noise:
    if use_so3:
        ...
        save_folder = os.path.join(args.save_root_path, 'AugMax_results/augmax_training', dataset_str, model_str, 'noise0.5_val')
else:
    if use_so3:
...
def val_cifar_worst_of_k_affine(K, model):
    ...
    with torch.no_grad():        
        ...
        K_logits = torch.zeros((K, args.test_batch_size, 1000)).cuda()
        for k in range(K):
            random.seed(k+1)
            # val_data = cifar_random_affine_test_set(data_dir=args.data_root_path, num_classes=10)
            val_data = imagenet_random_affine_test_set(data_dir=args.data_root_path, num_classes=1000)
...
def train(gpu_id, ngpus_per_node):
    ...
    if args.resume:
        ...
        scheduler.milestones = Counter({23:1})
        for i in range(23):
            scheduler.step()
            print(scheduler.get_last_lr()[0])
        print(scheduler.get_last_lr()[0], start_epoch)
    else:
        start_epoch = 0
        best_SA = 0
        ...
        if epoch < 0:
            ...
            for i, (images_tuples, labels) in enumerate(train_loader):
                ...
                if use_so3:
                    ...
                    rot, trans = SE2.exp(w)[0], torch.clamp(SE2.exp(w)[1], -30, 30.)
                ...
                for inter in range(5):
                    if add_noise:
                        w = w + 0.5 * (grad + 0.1 * torch.abs(grad).max() * torch.rand(grad.shape))
                    else:
                        w = w + 0.5 * grad
                    # clamp rotation here
                    ww = w.clone()
                    ...
                    if use_so3:
                        ...
                        rot, trans = SE2.exp(ww)[0], torch.clamp(SE2.exp(ww)[1], -30., 30.)
                        theta = torch.cat([rot, trans], 1).to(device)
