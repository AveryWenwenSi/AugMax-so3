...
if add_noise:
    if use_so3:
        ...
        save_folder = os.path.join(args.save_root_path, 'AugMax_results/augmax_training', dataset_str, model_str, 'add_noise_single_gd')
else:
    if use_so3:
        save_folder = os.path.join(args.save_root_path, 'AugMax_results/augmax_training', dataset_str, model_str, 'so3')
    else:
        save_folder = os.path.join(args.save_root_path, 'AugMax_results/augmax_training', dataset_str, model_str, 'no_noise_gdp')
...
def train(gpu_id, ngpus_per_node):
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for i, (images_tuples, labels) in enumerate(train_loader):
            ...
            data_combined, target_combined = images_tuples.detach().cpu().numpy(), labels
            w = Variable(torch.zeros([1, 3]), requires_grad=True)
            ...
            for _ in range(5):           
                ...
                # loss:
                loss_sample = F.cross_entropy(logits, labels)
                loss_sample.backward()
                grad = w.grad

                if add_noise:
                    w = w + (grad + 0.1 * grad.max() * torch.rand(grad.shape))
                else:
                    w = w + grad
                ...
                if use_so3:
                    ...
                    rot, trans = SE2.exp(w)[0], torch.clamp(SE2.exp(w)[1], -3., 3.)
                    theta = torch.cat([rot, trans], 1).to(device)      
                trans_data = spatial_sampler(trans_data, theta.repeat(trans_data.shape[0],1,1))
                data_combined = np.concatenate([data_combined, trans_data.detach().cpu().numpy()], 0)
                target_combined = torch.cat([target_combined, labels], 0)
                
            # # metrics:
            logits = model(torch.Tensor(data_combined).cuda())
            loss = F.cross_entropy(logits, target_combined)
            loss.backward()
            optimizer.step()

            l = logits.shape[0]
            accs.append((logits[:l//2].argmax(1) == labels).float().mean().item())
            accs_augmax.append((logits[l//2:].argmax(1) == target_combined[l//2:]).float().mean().item())
