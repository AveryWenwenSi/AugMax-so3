...
def train(gpu_id, ngpus_per_node):
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        # if epoch % 5 == 0:
            ...
            #     os.path.join(save_folder, 'latest{}.pth'.format(epoch)))
            # torch.save(model.state_dict(), os.path.join(save_folder, 'latest_origin{}.pth'.format(epoch)))
            # with open(os.path.join(save_folder, 'test.npy'), 'wb') as f:
            #     np.save(f, np.array(theta_list))
