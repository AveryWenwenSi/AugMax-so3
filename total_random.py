...
save_folder = os.path.join(args.save_root_path, 'AugMax_results/augmax_training', dataset_str, model_str, 'total_random_5steps')
# if add_noise:
#     if use_so3:
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for i, (images_tuples, labels) in enumerate(train_loader):
            ...
            for inter in range(5):
                ...
                w = 6*(torch.rand(w.shape) - 0.5)
            
                rot, trans = SE2.exp(w)[0], torch.clamp(SE2.exp(w)[1], -3., 3.)
                theta = torch.cat([rot, trans], 1).to(device)
                theta_list.append(w.detach().cpu().numpy())
