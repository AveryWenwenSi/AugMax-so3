...
def imagenet_dataloaders(data_dir, transform_train=True, AugMax=None, worst_of_k=False, **AugMax_args):
    ...
    if AugMax is not None: 
        ...
        if worst_of_k:
            test_transform = transforms.Compose(
                [transforms.Resize(256),
                transforms.CenterCrop(224),
                transforms.RandomAffine(degrees=30, translate=(30/224,30/224), shear=(-30,30)), preprocess])
                # transforms.RandomAffine(degrees=30, translate=(30/224,30/224), shear=(-30,30)), preprocess])
            return datasets.ImageFolder(validation_root, transform=test_transform)
